...
<script type="text/javascript">
	function fncGetList(currentPage) {
		...
		$("form").attr("method", "POST").attr("action", "/product/listProduct?"+$("#menu").val()).submit();
	...
	function getProduct(prodNo){
		...
		if($("#menu").val() == manage){
			$("form").attr("method", "POST").attr("action", "/product/updateProduct?"+prodNo).submit();
		}
		
		if($("#menu").val() == search){
			$("form").attr("method", "POST").attr("action", "/product/getProduct?"+prodNo).submit();
		}
	...
	$( ".ct_list_pop td:nth-child(3)" ).on("click" , function() {	
		getProduct(	$( ".ct_list_pop td:nth-child(1)" ).val()	);
	});
	...
	<div style="width: 98%; margin-left: 10px;">
		...
		<form name="detailForm">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0"
				...
				<c:forEach var="product" items="${list}">
					...
					<tr class="ct_list_pop">
						<td align="center" value="${product.prodNo}">${product.prodNo}</td>
						<td></td>
						<c:if test="${menu=='search'}">
						<td align="left"><a href="#">${product.prodName}</a></td>
						</c:if>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0"
				...
				<tr>
				<td align="center"><input type="hidden" id="menu" name="menu" value="${menu}"></td>				
				</tr> 
			</table>
			<!--  페이지 Navigator 끝 -->
